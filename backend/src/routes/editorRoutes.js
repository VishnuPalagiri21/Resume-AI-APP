...
/* ─────────────────────────────────────────────
   Utility: format a resume_documents row into
...
/* ─────────────────────────────────────────────
   ...
   Used by the frontend template picker.
───────────────────────────────────────────── */
...
/* ─────────────────────────────────────────────
   ...
   All documents for the logged-in user.
───────────────────────────────────────────── */
router.get("/documents", async (req, res) => {
  try {
    ...
    console.error("[editorRoutes] GET /documents error:", err.message);
...
router.post("/documents", async (req, res) => {
  try {
    ...
    console.error("[editorRoutes] POST /documents error:", err.message);
...
/* ─────────────────────────────────────────────
   ...
   Generate a full LaTeX resume tailored to a
...
router.post("/generate", async (req, res) => {
  try {
    ...
    console.error("[editorRoutes] /generate error:", err.message);
...
/* ─────────────────────────────────────────────
   ...
   Compiles the LaTeX source via local pdflatex
   with automatic Cloud TeX Engine fallback.
───────────────────────────────────────────── */
  ...
  try {
    ...
    execFile(
      ...
      async (err, stdout, stderr) => {
        ...
        try {
          console.log("[editorRoutes] Local pdflatex binary unavailable or failed. Using Cloud TeX Engine fallback...");
        ...
        if (!fs.existsSync(pdfFile))
          return res.status(422).json({ message: "PDF was not generated" });

        const pdfUrl = `/uploads/tmp/${doc.id}.pdf`;

        // Persist the compiled PDF URL
        await supabase
          .from("resume_documents")
          .update({ compiled_pdf_url: pdfUrl, updated_at: new Date().toISOString() })
          .eq("id", doc.id);

        res.json({ message: "Compiled successfully", pdfUrl });
      }
    );
  } catch (err) {
    console.error("[editorRoutes] compile error:", err.message);
...
/* ─────────────────────────────────────────────
   ...
   Save a named snapshot of the current LaTeX.
───────────────────────────────────────────── */
...
/* ─────────────────────────────────────────────
   Parse pdflatex stderr into structured errors
   for Monaco editor squiggles.
───────────────────────────────────────────── */
function parseLatexErrors(stderrText = "") {
  const errors = [];
  if (!stderrText) return errors;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("!")) {
      currentError = { message: line.replace(/^!\s*/, ""), lineNumber: 1, severity: "error" };
    }
    ...
    if (line.startsWith("LaTeX Warning:") || line.startsWith("Package Warning:")) {
      errors.push({ message: line, lineNumber: 1, severity: "warning" });
    }
  }
