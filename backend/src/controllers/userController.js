 ...
 * User Controller
 * 
 * Handles all job-seeker business logic: dashboard stats,
 * job browsing, job applications (with ATS scoring), and
 * application history with status audit trails.
 */
const multer           = require("multer");
const pdf              = require("pdf-parse");
const supabase         = require("../config/supabase");
const { analyzeResume } = require("../utils/atsEngine");
const { getStatusHistory } = require("../utils/statusHistory");
...
async function getJobs(req, res) {
  try {
    ...
    console.error("[userController] /jobs error:", err.message);
...
async function applyToJob(req, res) {
  try {
    const {
      resumeId, coverNote, fullName, email, phone,
      location, experienceYears, noticePeriod, linkedinUrl, portfolioUrl,
    } = req.body;
    ...
    console.error("[userController] /apply error:", err.message);
...
async function getApplications(req, res) {
  try {
    ...
    console.error("[userController] /applications error:", err.message);
    res.status(500).json({ message: "Failed to fetch applications" });
  }
}

module.exports = { upload, getStats, getJobs, getJobById, applyToJob, getApplications };
